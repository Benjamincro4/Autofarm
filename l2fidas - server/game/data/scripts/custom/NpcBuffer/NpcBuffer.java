...
import static fidas.server.gameserver.util.Util.formatAdena;
...
import fidas.server.gameserver.model.actor.instance.L2CubicInstance;
import fidas.server.gameserver.model.actor.instance.L2PcInstance;
import fidas.server.gameserver.model.actor.instance.L2PetInstance;
import fidas.server.gameserver.model.actor.instance.L2ServitorInstance;
import fidas.server.gameserver.model.olympiad.OlympiadManager;
import fidas.server.gameserver.model.actor.stat.PcStat;
import fidas.server.gameserver.model.actor.stat.SummonStat;
import fidas.server.gameserver.model.actor.status.PcStatus;
import fidas.server.gameserver.model.actor.status.SummonStatus;
...
import fidas.server.gameserver.network.serverpackets.SetSummonRemainTime;
...
public class NpcBuffer extends Quest
{
    ...
    private static final boolean ENABLE_VIP_BUFFER = false;//Config.NpcBuffer_VIP;
    //private static final int VIP_ACCESS_LEVEL = 8;//Config.NpcBuffer_VIP_ALV;
    private static final boolean ENABLE_BUFF_SECTION = true;//Config.NpcBuffer_EnableBuff;
    private static final boolean ENABLE_SCHEME_SYSTEM = true;//Config.NpcBuffer_EnableScheme;
    private static final boolean ENABLE_HEAL = true;//Config.NpcBuffer_EnableHeal;
    private static final boolean ENABLE_BUFFS = true;//Config.NpcBuffer_EnableBuffs;
    private static final boolean ENABLE_RESIST = true;//Config.NpcBuffer_EnableResist;
    private static final boolean ENABLE_SONGS = true;//Config.NpcBuffer_EnableSong;
    private static final boolean ENABLE_DANCES = true;//Config.NpcBuffer_EnableDance;
    private static final boolean ENABLE_CHANTS = true;//Config.NpcBuffer_EnableChant;
    private static final boolean ENABLE_OTHERS = true;//Config.NpcBuffer_EnableOther;
    private static final boolean ENABLE_SPECIAL = true;//Config.NpcBuffer_EnableSpecial;
    private static final boolean ENABLE_CUBIC = true;//Config.NpcBuffer_EnableCubic;
    private static final boolean ENABLE_BUFF_REMOVE = true;//Config.NpcBuffer_EnableCancel;
    private static final boolean ENABLE_BUFF_SET = true;//Config.NpcBuffer_EnableBuffSet;
    private static final boolean BUFF_WITH_KARMA = false;//Config.NpcBuffer_EnableBuffPK;
    private static final boolean FREE_BUFFS = false;//Config.NpcBuffer_EnableFreeBuffs;
    private static final boolean TIME_OUT = false;//Config.NpcBuffer_EnableTimeOut;
    private static final int TIME_OUT_TIME = Config.NpcBuffer_TimeOutTime;
    private static final int MIN_LEVEL = 1;//Config.NpcBuffer_MinLevel;
    private static final int BUFF_REMOVE_PRICE = 20000;//Config.NpcBuffer_PriceCancel;
    private static final int HEAL_PRICE = 20000;//Config.NpcBuffer_PriceHeal;
    private static final int BUFF_PRICE = 1000;//Config.NpcBuffer_PriceBuffs;
    private static final int RESIST_PRICE = 1000;//Config.NpcBuffer_PriceResist;
    private static final int SONG_PRICE = 1000;//Config.NpcBuffer_PriceSong;
    private static final int DANCE_PRICE = 1000;//Config.NpcBuffer_PriceDance;
    private static final int CHANT_PRICE = 1000;//Config.NpcBuffer_PriceChant;
    private static final int OTHERS_PRICE = 1000;//Config.NpcBuffer_PriceOther;
    private static final int SPECIAL_PRICE = 1000;//Config.NpcBuffer_PriceSpecial;
    private static final int CUBIC_PRICE = 0;//Config.NpcBuffer_PriceCubic;
    private static final int BUFF_SET_PRICE = 20000;//Config.NpcBuffer_PriceSet;
    private static final int SCHEME_BUFF_PRICE = 20000;//Config.NpcBuffer_PriceScheme;
    private static final int SCHEMES_PER_PLAYER = 20000;//Config.NpcBuffer_MaxScheme;
    private static final int CONSUMABLE_ID = 57;//Config.NpcBuffer_consumableID;
    private static final int MAX_SCHEME_BUFFS = 20;//Config.BUFFS_MAX_AMOUNT;
    private static final int MAX_SCHEME_DANCES = 12;//Config.DANCES_MAX_AMOUNT;
    ...
    private String rebuildMainHtml(QuestState st)
    {
        ...
        if (ENABLE_BUFF_SECTION)
        {
            ...
            //FIXME VIP BUFFS
            if (ENABLE_OTHERS && player!=null && player.isVip())
            {
                if (td>2) td=0;
                MESSAGE += TRS[td] + "<button value=\"VIP\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " redirect view_others 0 0\" width=130 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + TRS[td + 1];
                td+=2;
            }
            
            //FIXME VIP+ BUFFS
            if (ENABLE_CUBIC && player!=null && player.isAio())
            {            	
                if (td>2) td=0;
                MESSAGE += TRS[td] + "<button value=\"VIP+\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " redirect view_cubic 0 0\" width=130 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + TRS[td + 1];
                td+=2;
            }
        }
        ...
        else
        {
            ...
            else
            {
                ...
                switch (buffType)
                {
                    ...
                    case "others": price = OTHERS_PRICE; spiritNeed = true; break;                    
                    case "cubic": price = CUBIC_PRICE; noblesse = true; break;
                    default: if (DEBUG) throw new RuntimeException();
                }
                
                if(noblesse){
                	HTML_MESSAGE += "For <font color=LEVEL>Noblesse Blessing</font> you will need <font color=LEVEL>5 Spirit Ore</font>!";
                }else{
                    HTML_MESSAGE += "All buffs cost <font color=LEVEL>" + formatAdena(price) + "</font> adena!";
                }
                if(spiritNeed){
                	HTML_MESSAGE += "<br>Also for <font color=LEVEL>Cubics</font> you will need <font color=LEVEL>Spirit Ore</font>.";
                }
            }
    ...
    private String generateQuery(int case1, int case2, L2PcInstance player)
    {
        ...
        //VIP
        if(player!=null && player.getVipEndTime() > System.currentTimeMillis()){ // player.getAccessLevel().getLevel() == VIP_ACCESS_LEVEL){
    ...
    private String viewAllSchemeBuffs$getBuffCount(String scheme)
    {
        ...
        try (Connection con = L2DatabaseFactory.getInstance().getConnection())
        {
            PreparedStatement rss = con.prepareStatement("SELECT skill_id, buff_class FROM npcbuffer_scheme_contents WHERE scheme_id=?");
            rss.setString(1, scheme);
            ResultSet action = rss.executeQuery();
            while (action.next())
            {
                ++count;
                int skillId = action.getInt("skill_id");
                int val = action.getInt("buff_class");
                
                if ((skillId == 67) || (skillId == 10) || (skillId == 22) || (skillId == 33) || (skillId == 278) || (skillId == 449) || (skillId == 779) || (skillId == 1279) || (skillId == 1280) || (skillId == 1281)) // NO CUENTA (NI COMO BUFF NI DANZA) - Los 10 cubics
                {
                    continue;
                }

                if ((skillId == 915) || (val == 2) || (val == 4)) // CUENTA COMO DANZA/SONG (buff_class 2 y 4) - skillId == 915 Dance Berserker. Tiene categoria buff pero tiene que contar como danza
                {
                	++D_S_Count;
                }
                else // CUENTA COMO BUFF
                {
                	++B_Count;
                }
            }
    ...
    private String viewAllSchemeBuffs(String scheme, String page, String action, L2PcInstance player)
    {
        ...
        try (Connection con = L2DatabaseFactory.getInstance().getConnection())
        {
            if (action.equals("add"))
            {
                ...
                String QUERY = "SELECT * FROM npcbuffer_buff_list WHERE buffType IN (" + generateQuery(BUFF_COUNT, DANCE_SONG, player) + ") AND buffId NOT IN(67,10,22,33,278,449,1279,1280,1281,779,4699,4700,4702,4703,825,826,827,828,829,830,1364,1416,1542,915) AND canUse=1 ORDER BY Buff_Class ASC, id";
                if(player!=null && player.getVipEndTime() > System.currentTimeMillis()){ //getAccessLevel().getLevel() == VIP_ACCESS_LEVEL){
                	QUERY = "SELECT * FROM npcbuffer_buff_list WHERE buffType IN (" + generateQuery(BUFF_COUNT, DANCE_SONG, player) + ") AND canUse=1 ORDER BY Buff_Class ASC, id";
                }
    ...
    private String viewAllBuffTypes()
    {
        ...
        if (ENABLE_OTHERS)
        {
        	HTML_MESSAGE += "<button value=\"VIP\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " edit_buff_list others Others_Buffs 1\" width=200 height=22 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">";
        }
        if (ENABLE_CUBIC)
        {
        	HTML_MESSAGE += "<button value=\"VIP+\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " edit_buff_list cubic cubic_Buffs 1\" width=200 height=22 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">";
        }
        if (ENABLE_BUFF_SET)
        {
    ...
    private String viewAllBuffs(String type, String typeName, String page, L2PcInstance player)
    {
        List<String> buffList = new ArrayList<>();
        String HTML_MESSAGE = "<html><head><title>" + TITLE_NAME + "</title></head><body><center><img src=\"L2UI_CH3.herotower_deco\" width=256 height=32><br>";
        ...
        try (Connection con = L2DatabaseFactory.getInstance().getConnection())
        {
            ...
            if (type.equals("set"))
            {
                getBuffCount = con.prepareStatement("SELECT * FROM npcbuffer_buff_list WHERE buffType IN (" + generateQuery(0, 0, player) + ") AND canUse=1");
            }
            else
            {
    ...
    private String manageSelectedSet(String id, String newVal, String opt3, L2PcInstance player)
    {
        ...
        return viewAllBuffs("set", "Buff Sets", opt3, player);
    }
    ...
    /*--String[] eventSplit = event.split(" ");--*/
        ...
        switch (eventParam0)
        {
            ...
            case "manage_scheme_1": return viewAllSchemeBuffs(eventParam1, eventParam2, "add", player);
            case "manage_scheme_2": return viewAllSchemeBuffs(eventParam1, eventParam2, "remove", player);
            ...
            case "remove_buff":
            {
                ...
                else HTML = viewAllSchemeBuffs(scheme, eventParam2, "remove", player);
                return HTML;
            }
            ...
            case "add_buff":
            {
                ...
                else HTML = viewAllSchemeBuffs(scheme, eventParam2, "add", player);
                return HTML;
            }

            case "edit_buff_list":
                return viewAllBuffs(eventParam1, eventParam2, eventParam3, player);
            ...
            case "changeBuffSet":
                ...
                return manageSelectedSet(eventParam1, eventParam2, eventParam3, player);
            ...
            case "editSelectedBuff":
            {
                ...
                return viewAllBuffs(eventParam3, typeName, page, player);
            }
            ...
            case "cast":
                if ((int)(System.currentTimeMillis()/1000) > st.getInt("blockUntilTime"))
                {
                  ...
                  //catch (SQLException e) { print "Query error!"; } //TODO:
                    ...
                    if (buffs.size() == 0)
                    {
                    	return viewAllSchemeBuffs(eventParam1, "1", "add", player);
                    }
					if (!FREE_BUFFS)
					{
					...
					for (int i = 0; i < buffs.size(); ++i)
					{
					    if (!getpetbuff)
					    {
		                    L2Skill skill = SkillTable.getInstance().getInfo(buffs.get(i), levels.get(i));
					    	if (skill.getSkillType() == L2SkillType.SUMMON/*eventParam3.equals("cubic")*/)
	                        {
	                            if (player.getCubics() != null)
	                            {
	                                for (L2CubicInstance cubic : player.getCubics().values())
	                            ...
	                            player.useMagic(SkillTable.getInstance().getInfo(buffs.get(i), levels.get(i)), false, false);
	                        }
	                        else if(skill.getId() == 1323) //es noblesse
	                        {
	                        ...
	                        {
	                        	if(player.getAccessLevel().getLevel()!=8 && (skill.getId() == 825 || skill.getId() == 826 || skill.getId() == 827 || skill.getId() == 828 || skill.getId() == 829 || skill.getId() == 830 || skill.getId() == 4699 || skill.getId() == 4700 || skill.getId() == 4702 || skill.getId() == 4703)){
	                        	}else{
            ...
            case "giveBuffs":
            {
                ...
                if ((int)(System.currentTimeMillis()/1000) > st.getInt("blockUntilTime"))
                {
                    if (!FREE_BUFFS)
                    {
                        ...
                        {
                        	return showText(st, "Sorry", "You don't have the enough adena:<br>You need: <font color=LEVEL>" + cost + " " + getItemNameHtml(st, CONSUMABLE_ID) + "!", false, "0", "0");
                        }
                    }
                    L2Skill skill = SkillTable.getInstance().getInfo(Integer.parseInt(eventParam1), Integer.parseInt(eventParam2));
                    if (skill.getSkillType() == L2SkillType.SUMMON || skill.getId() == 1323)
                    {
	...
	public String onFirstTalk(L2Npc npc, L2PcInstance player)
    {
        ...
        removeVipBuffsFromPlayerSchemes(player);
    ...
    private void removeVipBuffsFromPlayerSchemes(L2PcInstance player)
    {
        if (player == null)
        {
            return;
        ...
        if (player.getVipEndTime() > System.currentTimeMillis())
        {
            return;
        }
        ...
        try (Connection con = L2DatabaseFactory.getInstance().getConnection();
            PreparedStatement ps = con.prepareStatement(
                 "DELETE sc FROM npcbuffer_scheme_contents sc INNER JOIN npcbuffer_scheme_list sl ON sl.id = sc.scheme_id WHERE sl.player_id = ? AND sc.buff_class = ?;"
            )
        )
        {

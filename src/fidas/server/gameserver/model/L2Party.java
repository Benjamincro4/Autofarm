...
import fidas.server.gameserver.model.entity.DimensionalRift;
...
import fidas.server.gameserver.network.serverpackets.ExAskModifyPartyLooting;
import fidas.server.gameserver.network.serverpackets.ExCloseMPCC;
import fidas.server.gameserver.network.serverpackets.ExOpenMPCC;
...
import fidas.server.gameserver.network.serverpackets.ExSetPartyLooting;
import fidas.server.gameserver.network.serverpackets.L2GameServerPacket;
import fidas.server.gameserver.network.serverpackets.PartyMemberPosition;
import fidas.server.gameserver.network.serverpackets.PartySmallWindowAdd;
import fidas.server.gameserver.network.serverpackets.PartySmallWindowAll;
import fidas.server.gameserver.network.serverpackets.PartySmallWindowDelete;
import fidas.server.gameserver.network.serverpackets.PartySmallWindowDeleteAll;
...
public class L2Party extends AbstractPlayerGroup
{
	 ...
	 * @see fidas.server.gameserver.model.actor.instance.L2PcInstance#isRequestExpired()
	 */
		...
		synchronized (rewardedMembers)
		{
			for (L2Character member : rewardedMembers)
			{
				...
				if (validMembers.contains(member))
				{
					...
					if (member.isPlayer())
					{
					    final L2PcInstance player = member.getActingPlayer();
					    
					    //FIXME VIP
					    if (player.isVip() && Config.ALLOW_VIP_EXP_SP)
					    {
					        addexp = Math.round(addexp * Config.VIP_XP);
					        addsp = Math.round(addsp * Config.VIP_SP);
					    }

					    addexp = calculateExpSpPartyCutoff(player, topLvl, addexp, addsp, useVitalityRate);

					    final int skillLvl = player.getSkillLevel(467);
					    if (skillLvl > 0)
					    {
					    ...
					    if (addexp > 0)
					    {
					        ...
					        PcCafePointsManager.getInstance().givePcCafePoint(player, addexp);
					    }
					}
					else
					{

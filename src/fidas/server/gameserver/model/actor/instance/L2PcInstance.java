 ...
 */
package fidas.server.gameserver.model.actor.instance;
...
import fidas.server.gameserver.GameTimeController;
import fidas.server.gameserver.GeoData;
import fidas.server.gameserver.ItemsAutoDestroy;
import fidas.server.gameserver.LoginServerThread;
import fidas.server.gameserver.RecipeController;
...
import fidas.server.gameserver.ai.L2CharacterAI;
import fidas.server.gameserver.ai.L2PlayerAI;
import fidas.server.gameserver.ai.L2SummonAI;
import fidas.server.gameserver.cache.HtmCache;
import fidas.server.gameserver.cache.WarehouseCacheManager;
import fidas.server.gameserver.communitybbs.BB.Forum;
import fidas.server.gameserver.communitybbs.Manager.ForumsBBSManager;
...
import fidas.server.gameserver.datatables.AdventBonus;
...
import fidas.server.gameserver.datatables.SkillTable.FrequentSkill;
...
import fidas.server.gameserver.handler.ItemHandler;
...
import fidas.server.gameserver.instancemanager.DuelManager;
import fidas.server.gameserver.instancemanager.ExpirableServicesManager;
import fidas.server.gameserver.instancemanager.ExpirableServicesManager.ServiceType;
...
import fidas.server.gameserver.instancemanager.HandysBlockCheckerManager;
...
import fidas.server.gameserver.model.L2Account;
import fidas.server.gameserver.model.L2Clan;
import fidas.server.gameserver.model.L2ClanMember;
import fidas.server.gameserver.model.L2ContactList;
import fidas.server.gameserver.model.L2EnchantSkillLearn;
import fidas.server.gameserver.model.L2Macro;
import fidas.server.gameserver.model.L2ManufactureItem;
import fidas.server.gameserver.model.L2ManufactureList;
import fidas.server.gameserver.model.L2Object;
import fidas.server.gameserver.model.L2Party;
import fidas.server.gameserver.model.L2Party.messageType;
import fidas.server.gameserver.model.L2PetData;
import fidas.server.gameserver.model.L2PetLevelData;
import fidas.server.gameserver.model.L2PremiumItem;
import fidas.server.gameserver.model.L2Radar;
import fidas.server.gameserver.model.L2RecipeList;
import fidas.server.gameserver.model.L2Request;
import fidas.server.gameserver.model.L2ShortCut;
import fidas.server.gameserver.model.L2SkillLearn;
import fidas.server.gameserver.model.L2Transformation;
import fidas.server.gameserver.model.L2UIKeysSettings;
import fidas.server.gameserver.model.L2World;
import fidas.server.gameserver.model.L2WorldRegion;
import fidas.server.gameserver.model.Location;
import fidas.server.gameserver.model.MacroList;
import fidas.server.gameserver.model.PartyMatchRoom;
...
import fidas.server.gameserver.model.ShortCuts;
import fidas.server.gameserver.model.ShotType;
import fidas.server.gameserver.model.TerritoryWard;
import fidas.server.gameserver.model.TimeStamp;
import fidas.server.gameserver.model.TradeList;
import fidas.server.gameserver.model.actor.L2Attackable;
import fidas.server.gameserver.model.actor.L2Character;
import fidas.server.gameserver.model.actor.L2Decoy;
import fidas.server.gameserver.model.actor.L2Npc;
import fidas.server.gameserver.model.actor.L2Playable;
import fidas.server.gameserver.model.actor.L2Summon;
import fidas.server.gameserver.model.actor.L2Trap;
import fidas.server.gameserver.model.actor.L2Vehicle;
import fidas.server.gameserver.model.actor.appearance.PcAppearance;
import fidas.server.gameserver.model.actor.knownlist.PcKnownList;
import fidas.server.gameserver.model.actor.position.PcPosition;
import fidas.server.gameserver.model.actor.stat.PcStat;
import fidas.server.gameserver.model.actor.status.PcStatus;
import fidas.server.gameserver.model.actor.templates.L2PcTemplate;
import fidas.server.gameserver.model.base.ClassId;
import fidas.server.gameserver.model.base.ClassLevel;
import fidas.server.gameserver.model.base.PlayerClass;
import fidas.server.gameserver.model.base.Race;
import fidas.server.gameserver.model.base.Sex;
import fidas.server.gameserver.model.base.SubClass;
import fidas.server.gameserver.model.effects.AbnormalEffect;
import fidas.server.gameserver.model.effects.EffectFlag;
...
import fidas.server.gameserver.model.entity.Duel;
...
import fidas.server.gameserver.model.entity.Instance;
...
import fidas.server.gameserver.model.fishing.L2Fish;
import fidas.server.gameserver.model.fishing.L2Fishing;
import fidas.server.gameserver.model.itemcontainer.Inventory;
import fidas.server.gameserver.model.itemcontainer.ItemContainer;
import fidas.server.gameserver.model.itemcontainer.PcFreight;
import fidas.server.gameserver.model.itemcontainer.PcInventory;
import fidas.server.gameserver.model.itemcontainer.PcRefund;
import fidas.server.gameserver.model.itemcontainer.PcWarehouse;
import fidas.server.gameserver.model.itemcontainer.PetInventory;
import fidas.server.gameserver.model.items.L2Armor;
import fidas.server.gameserver.model.items.L2EtcItem;
import fidas.server.gameserver.model.items.L2Henna;
import fidas.server.gameserver.model.items.L2Item;
import fidas.server.gameserver.model.items.L2Weapon;
import fidas.server.gameserver.model.items.instance.L2ItemInstance;
import fidas.server.gameserver.model.items.type.L2ActionType;
import fidas.server.gameserver.model.items.type.L2ArmorType;
import fidas.server.gameserver.model.items.type.L2EtcItemType;
import fidas.server.gameserver.model.items.type.L2WeaponType;
import fidas.server.gameserver.model.multisell.PreparedListContainer;
import fidas.server.gameserver.model.olympiad.OlympiadGameManager;
import fidas.server.gameserver.model.olympiad.OlympiadGameTask;
import fidas.server.gameserver.model.olympiad.OlympiadManager;
import fidas.server.gameserver.model.quest.Quest;
import fidas.server.gameserver.model.quest.Quest.QuestEventType;
...
import fidas.server.gameserver.model.skills.l2skills.L2SkillSiegeFlag;
import fidas.server.gameserver.model.skills.l2skills.L2SkillSummon;
import fidas.server.gameserver.model.skills.l2skills.L2SkillTrap;
import fidas.server.gameserver.model.skills.targets.L2TargetType;
import fidas.server.gameserver.model.stats.Env;
import fidas.server.gameserver.model.stats.Formulas;
import fidas.server.gameserver.model.stats.Rates;
import fidas.server.gameserver.model.stats.Stats;
import fidas.server.gameserver.model.zone.L2ZoneType;
import fidas.server.gameserver.model.zone.ZoneId;
import fidas.server.gameserver.model.zone.type.L2BossZone;
import fidas.server.gameserver.model.zone.type.L2NoRestartZone;
import fidas.server.gameserver.network.DialogId;
...
import fidas.server.gameserver.network.serverpackets.ActionFailed;
import fidas.server.gameserver.network.serverpackets.ChangeWaitType;
import fidas.server.gameserver.network.serverpackets.CharInfo;
import fidas.server.gameserver.network.serverpackets.ConfirmDlg;
import fidas.server.gameserver.network.serverpackets.EtcStatusUpdate;
import fidas.server.gameserver.network.serverpackets.ExAutoSoulShot;
import fidas.server.gameserver.network.serverpackets.ExBasicActionList;
import fidas.server.gameserver.network.serverpackets.ExBrExtraUserInfo;
import fidas.server.gameserver.network.serverpackets.ExDominionWarStart;
import fidas.server.gameserver.network.serverpackets.ExDuelUpdateUserInfo;
import fidas.server.gameserver.network.serverpackets.ExFishingEnd;
import fidas.server.gameserver.network.serverpackets.ExFishingStart;
import fidas.server.gameserver.network.serverpackets.ExGetBookMarkInfoPacket;
import fidas.server.gameserver.network.serverpackets.ExGetOnAirShip;
...
import fidas.server.gameserver.network.serverpackets.ExOlympiadMode;
import fidas.server.gameserver.network.serverpackets.ExPrivateStoreSetWholeMsg;
import fidas.server.gameserver.network.serverpackets.ExRedSky;
import fidas.server.gameserver.network.serverpackets.ExSetCompassZoneCode;
import fidas.server.gameserver.network.serverpackets.ExSpawnEmitter;
import fidas.server.gameserver.network.serverpackets.ExStartScenePlayer;
import fidas.server.gameserver.network.serverpackets.ExStorageMaxCount;
import fidas.server.gameserver.network.serverpackets.ExUseSharedGroupItem;
import fidas.server.gameserver.network.serverpackets.ExVitalityPointInfo;
import fidas.server.gameserver.network.serverpackets.ExVoteSystemInfo;
import fidas.server.gameserver.network.serverpackets.FriendStatusPacket;
import fidas.server.gameserver.network.serverpackets.GameGuardQuery;
import fidas.server.gameserver.network.serverpackets.GetOnVehicle;
import fidas.server.gameserver.network.serverpackets.HennaInfo;
import fidas.server.gameserver.network.serverpackets.InventoryUpdate;
import fidas.server.gameserver.network.serverpackets.ItemList;
import fidas.server.gameserver.network.serverpackets.L2GameServerPacket;
import fidas.server.gameserver.network.serverpackets.LeaveWorld;
import fidas.server.gameserver.network.serverpackets.MagicSkillUse;
import fidas.server.gameserver.network.serverpackets.NicknameChanged;
import fidas.server.gameserver.network.serverpackets.NpcHtmlMessage;
import fidas.server.gameserver.network.serverpackets.ObservationMode;
import fidas.server.gameserver.network.serverpackets.ObservationReturn;
import fidas.server.gameserver.network.serverpackets.PartySmallWindowUpdate;
import fidas.server.gameserver.network.serverpackets.PetInventoryUpdate;
import fidas.server.gameserver.network.serverpackets.PlaySound;
import fidas.server.gameserver.network.serverpackets.PledgeShowMemberListDelete;
import fidas.server.gameserver.network.serverpackets.PledgeShowMemberListUpdate;
import fidas.server.gameserver.network.serverpackets.PrivateStoreListBuy;
import fidas.server.gameserver.network.serverpackets.PrivateStoreListSell;
import fidas.server.gameserver.network.serverpackets.PrivateStoreManageListBuy;
import fidas.server.gameserver.network.serverpackets.PrivateStoreManageListSell;
import fidas.server.gameserver.network.serverpackets.PrivateStoreMsgBuy;
import fidas.server.gameserver.network.serverpackets.PrivateStoreMsgSell;
import fidas.server.gameserver.network.serverpackets.RecipeShopMsg;
import fidas.server.gameserver.network.serverpackets.RecipeShopSellList;
import fidas.server.gameserver.network.serverpackets.RelationChanged;
import fidas.server.gameserver.network.serverpackets.Ride;
import fidas.server.gameserver.network.serverpackets.ServerClose;
import fidas.server.gameserver.network.serverpackets.SetupGauge;
import fidas.server.gameserver.network.serverpackets.ShortBuffStatusUpdate;
...
import fidas.server.gameserver.network.serverpackets.Snoop;
import fidas.server.gameserver.network.serverpackets.SocialAction;
import fidas.server.gameserver.network.serverpackets.StatusUpdate;
import fidas.server.gameserver.network.serverpackets.StopMove;
import fidas.server.gameserver.network.serverpackets.SystemMessage;
import fidas.server.gameserver.network.serverpackets.TargetSelected;
import fidas.server.gameserver.network.serverpackets.TargetUnselected;
import fidas.server.gameserver.network.serverpackets.TradeDone;
import fidas.server.gameserver.network.serverpackets.TradeOtherDone;
import fidas.server.gameserver.network.serverpackets.TradeStart;
import fidas.server.gameserver.network.serverpackets.UserInfo;
import fidas.server.gameserver.scripting.scriptengine.events.EquipmentEvent;
import fidas.server.gameserver.scripting.scriptengine.events.HennaEvent;
import fidas.server.gameserver.scripting.scriptengine.events.ProfessionChangeEvent;
import fidas.server.gameserver.scripting.scriptengine.events.TransformEvent;
import fidas.server.gameserver.scripting.scriptengine.listeners.player.EquipmentListener;
import fidas.server.gameserver.scripting.scriptengine.listeners.player.HennaListener;
import fidas.server.gameserver.scripting.scriptengine.listeners.player.PlayerDespawnListener;
import fidas.server.gameserver.scripting.scriptengine.listeners.player.ProfessionChangeListener;
import fidas.server.gameserver.scripting.scriptengine.listeners.player.TransformListener;
import fidas.server.gameserver.taskmanager.AttackStanceTaskManager;
import fidas.server.gameserver.util.BotPunish;
import fidas.server.gameserver.util.FloodProtectors;
import fidas.server.gameserver.util.PlayerEventStatus;
import fidas.server.gameserver.util.Point3D;
import fidas.server.gameserver.util.Util;
import fidas.server.util.L2FastList;
import fidas.server.util.Rnd;
import fidas.server.util.TimeConstant;
...
public final class L2PcInstance extends L2Playable
{
	...
	// Autopotion setAutoPotionConfig
	public void setAutoPotionConfig(int health, int mana, int cp, boolean enabled) {
		this.healthThreshold = health;
		this.manaThreshold = mana;
		this.cpThreshold = cp;
	    this.autoPotionEnabled = enabled;
	    saveAutopotionConfig(health, mana, cp, enabled);
	}
	
	// Autopotion getAPConfiguredHp
	public String getAPConfiguredHp() {
	    return String.valueOf(healthThreshold);	    
	}
	
	// Autopotion getAPConfiguredMp
	public String getAPConfiguredMp() {
	    return String.valueOf(manaThreshold);	    
	}
	
	// Autopotion getAPConfiguredCp
	public String getAPConfiguredCp() {
	    return String.valueOf(cpThreshold);
	}

	// Autopotion isAutoPotionEnabled
	public boolean isAutoPotionEnabled() {
	    return autoPotionEnabled;
	}
	
	// Autopotion chek and use Potion
	public void checkAndUseAutoPotion() {
	    if (!isAutoPotionEnabled() || isDead() || !isOnline()) {
	        return;
	    }

	    long currentTime = System.currentTimeMillis();
	    if (currentTime - lastPotionUse < POTION_COOLDOWN) {
	        return; // Evita el uso excesivo
	    }

	    double currentHpPercent = (getCurrentHp() / getMaxHp()) * 100;
	    double currentMpPercent = (getCurrentMp() / getMaxMp()) * 100;
	    double currentCpPercent = (getCurrentCp() / getMaxCp()) * 100;

	    if (currentHpPercent <= healthThreshold) {
	        if (usePotion(1539)) { // Greater Healing Potion ID
	            lastPotionUse = currentTime;
	        }
	    }

	    if (currentMpPercent <= manaThreshold) {
	        if (usePotion(728)) { // Mana Potion ID
	            lastPotionUse = currentTime;
	        }
	    }
	    
	    if (currentCpPercent <= cpThreshold) {
	        if (usePotion(5592)) { // Greater CP Potion ID
	            lastPotionUse = currentTime;
	        }
	    }
	}

	//Autopotion loadAutopotionConfig
	public void loadAutopotionConfig() {
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			String sql = "SELECT autopotion_config FROM characters WHERE charId=?";
			PreparedStatement statement = con.prepareStatement(sql);
			statement.setInt(1, getObjectId());
			ResultSet rset = statement.executeQuery();
			while (rset.next())
			{
				String autopotionConfig = rset.getString("autopotion_config");
				int configHp =  Integer.parseInt(parseAutopotionConfig(autopotionConfig, "hp"));
				int configMp =  Integer.parseInt(parseAutopotionConfig(autopotionConfig, "mp"));
				int configCp =  Integer.parseInt(parseAutopotionConfig(autopotionConfig, "cp"));
				boolean isEnable =  Boolean.parseBoolean(parseAutopotionConfig(autopotionConfig, "isEnable"));
				this.setAutoPotionConfig(configHp, configMp, configCp, isEnable);
			}
			rset.close();
			statement.close();
		}
		...
		{
			_log.log(Level.SEVERE, "Could not get autopotion configuration from bbdd: " + e.getMessage(), e);
		}
	}
	
	//Autopotion saveAutopotionConfig
	public void saveAutopotionConfig(int hp, int mp, int cp, boolean isEnable) {
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			String sql = "UPDATE characters SET autopotion_config=? WHERE charId=?";
			PreparedStatement statement = con.prepareStatement(sql);
			statement.setString(1, String.valueOf(hp) + ";" + String.valueOf(mp) + ";" + String.valueOf(cp) + ";" + String.valueOf(isEnable));
		...
		{
			_log.log(Level.SEVERE, "Could not save autopotion configuration on bbdd: " + e.getMessage(), e);
		}
	}
	
	//Autopotion parseAutopotionConfig
	public String parseAutopotionConfig(String autopotionConfig, String type) {
		String result = "";
		if(autopotionConfig != null && !autopotionConfig.isEmpty() && type != null && !type.isEmpty()){
			String[] config = autopotionConfig.split(";");
	...
	//Autopotion usePotion
	public boolean usePotion(int itemId) {
	    // Encuentra el ítem en el inventario
		L2ItemInstance potion = getInventory().getItemByItemId(itemId);
	    ...
	    // Usar el ítem como lo haría el cliente
	    IItemHandler handler = ItemHandler.getInstance().getHandler(potion.getEtcItem());
	    if (handler != null) {
	        handler.useItem(this, potion, false); // Usar el ítem (false = no forzar desde inventario)
	        return true;
	    }else {
	        sendMessage("No handler found for this item.");
	        return false;
	    }
	}
	...
	/** Vip System */
	private boolean _isVip = true;
	private long _vip_endTime = 30;
		...
		if (count > 0)
		{
			...
			if (sendMessage && ((!isCastingNow() && (item.getItemType() == L2EtcItemType.HERB)) || (item.getItemType() != L2EtcItemType.HERB)))
			{
				if (count > 1)
				{
					...
					{
						SystemMessage sm = null;
						L2PcInstance player = (L2PcInstance) reference;
						//FIXME: Autospoil falta activar el accesslevel definitivo
						if(player != null && process.equalsIgnoreCase("sweep") && player.getActingPlayer().isAio()){
							//sm = SystemMessage.getSystemMessage(SystemMessageId.REWARD_VIP_S2_S1_S);	`
				...
				else
				{
					if (process.equalsIgnoreCase("sweep") || process.equalsIgnoreCase("Quest"))
					{
						...
						//FIXME: Autospoil falta activar el accesslevel definitivo
						if(player != null && process.equalsIgnoreCase("sweep") && player.getActingPlayer().isAio()){
							//sm = SystemMessage.getSystemMessage(SystemMessageId.REWARD_VIP_ITEM_S1);	
			...
			PreparedStatement statement = con.prepareStatement(RESTORE_CHARACTER))
		{
			...
			try (ResultSet rset = statement.executeQuery())
			{
				if (rset.next())
				{
					...
					//Autopotion & Autofarm Load
					player.loadAutopotionConfig();
					//player.loadAutofarmConfig();
			...
			// DressMe equipar armadura al logar
			DressMeService.restoreEquippedArmorSet(player);
			
			// DressMe actualizar color nombre/titulo al logar
			DressMeService.restoreColorData(player);
		}
		catch (Exception e)
		{
	...
	public void rewardVipSkills()
	{
		...
		if(Config.VIP_SKILLS.keySet().size() > 0){
	...
	public void rewardAioSkills()
	{
		...
		if(Config.AIO_SKILLS.keySet().size()>0){
			sendMessage("[Vip+ System]: Admin has given you Vip+ skills.");
		}
	}
	...
	public void checkAndUseAutoFarm() {
	    if (this.isAutoFarmEnabled() && !this.isDead()) {
	        // Si ya está atacando, no busques un nuevo mob
	        if (this.isAttackingNow()) {
	            this.sendMessage("[AutoFarm] Ya estás atacando. Esperando a terminar.");
	            return; // No realizar más acciones mientras ataca
	        }

	        // Si el target actual es válido, sigue atacándolo
	        if (this.getTarget() instanceof L2Attackable) {
	            L2Attackable currentTarget = (L2Attackable) this.getTarget();
	            if (!currentTarget.isDead() && currentTarget.isAutoAttackable(this)) {
	                this.sendMessage("[AutoFarm] Continuando ataque al mob: " + currentTarget.getName());
	                useRandomAutoFarmSkill(currentTarget);
	                return;
	            }
	        }

	        // Buscar un nuevo mob si no hay target válido
	        L2Attackable target = findClosestMob(this);
	        if (target != null) {
	            this.setTarget(target);
	            this.sendMessage("[AutoFarm] Mob seleccionado: " + target.getName());
	            target.onAction(this); // Usar onAction en lugar de doAttack
	            this.sendMessage("[AutoFarm] Iniciando ataque a: " + target.getName());
	            
	            // Usar una habilidad aleatoria sobre el target
	            useRandomAutoFarmSkill(target);
	        } else {
	            this.sendMessage("[AutoFarm] No se encontró un mob cercano para atacar.");
	        }
	    }
	}
	private void useRandomAutoFarmSkill(L2Attackable target) {
	    if (target == null || target.isDead()) {
	        return; // No hacer nada si el target no es válido
	    }

	    // Obtener las habilidades seleccionadas en AutoFarm
	    Map<String, Integer> autoFarmSkills = this.getAutoFarmSkills();
	    if (autoFarmSkills.isEmpty()) {
	        this.sendMessage("[AutoFarm] No hay habilidades seleccionadas. Usando ataque básico.");
	        this.doAttack(target);
	        return;
	    }

	    // Seleccionar una habilidad aleatoria
	    List<Integer> skillIds = new ArrayList<>(autoFarmSkills.values());
	    int randomSkillId = skillIds.get(Rnd.get(skillIds.size())); // Selección aleatoria

	    // Obtener la habilidad del jugador
	    L2Skill skill = SkillTable.getInstance().getInfo(randomSkillId, this.getSkillLevel(randomSkillId));
	    if (skill != null) {
	        this.sendMessage("[AutoFarm] Usando habilidad: " + skill.getName() + " sobre " + target.getName());
	        this.useMagic(skill, false, false); // Ejecutar la habilidad
	    } else {
	        this.sendMessage("[AutoFarm] Error: No se encontró la habilidad con ID: " + randomSkillId);
	    }
	}
	...
	private static L2Attackable findClosestMob(L2PcInstance player) {
	    player.sendMessage("[AutoFarm] Ejecutando findClosestMob...");

	    // Verificar si hay objetos conocidos
	    if (player.getKnownList().getKnownCharacters().isEmpty()) {
	        player.sendMessage("[AutoFarm] No hay objetos conocidos en el radio de visión.");
	        return null;
	    }

	    // Depuración: Imprimir objetos conocidos
	    player.sendMessage("[AutoFarm] Objetos conocidos:");
	    for (L2Character obj : player.getKnownList().getKnownCharacters()) {
	        player.sendMessage(String.format(
	            "Nombre: %s, Tipo: %s, Vivo: %s, Atacable: %s",
	            obj.getName(),
	            obj.getClass().getSimpleName(),
	            !obj.isDead() ? "Sí" : "No",
	            (obj instanceof L2Attackable ? ((L2Attackable) obj).isAutoAttackable(player) : "N/A")
	        ));
	    }

	    // Buscar el mob más cercano
	    ...
	    for (L2Character obj : player.getKnownList().getKnownCharacters()) {
	        if (obj instanceof L2Attackable) {
	            ...
	            if (!mob.isDead() && mob.isAutoAttackable(player)) {
	                double distance = player.getDistanceSq(mob);
	    ...
	    if (closestMob == null) {
	        player.sendMessage("[AutoFarm] No se encontraron mobs atacables en el radio de visión.");
	    } else {
	        player.sendMessage("[AutoFarm] Mob más cercano encontrado: " + closestMob.getName() + " a una distancia de " + Math.sqrt(closestDistance));
	    }
	...
	public void loadAutofarmConfig() {
		...
		//Autofarm saveAutofarmConfig  Enable/disable - Assist pt leader / Use random Skills / keep starting farm Location / Radius
	public void saveAutofarmConfig() {
	    Connection con = null;
	    try {
	        con = L2DatabaseFactory.getInstance().getConnection();
	        PreparedStatement statement = con.prepareStatement("UPDATE characters SET autofarm_config=? WHERE charId=?");
	        statement.setString(1, assistToPartyLeader + ";" + useRandomSkills + ";" + keepStartingFarmLocation + ";" + autoFarmRadius);
	        statement.setInt(2, getObjectId());
	        statement.executeUpdate();
	        statement.close();
	    } catch (Exception e) {
	        _log.warning("Error al guardar la configuración del AutoFarm para el personaje " + getName() + ": " + e.getMessage());
	    } 
	}
		...
		public void parseAutofarmConfig(String config) {
		    ...
		    if (parts.length == 4) {
		        this.assistToPartyLeader = Boolean.parseBoolean(parts[0]);
		        this.useRandomSkills = Boolean.parseBoolean(parts[1]);
		        this.keepStartingFarmLocation = Boolean.parseBoolean(parts[2]);
		        try {
		        	this.autoFarmRadius = Integer.parseInt(parts[3]);
		        } catch (NumberFormatException e) {
		        	this.autoFarmRadius = 0;
		        }
		    }
		}

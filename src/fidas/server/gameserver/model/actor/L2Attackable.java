...
import fidas.server.gameserver.ai.L2FortSiegeGuardAI;
import fidas.server.gameserver.ai.L2SiegeGuardAI;
import fidas.server.gameserver.datatables.ChampionData;
import fidas.server.gameserver.datatables.EventDroplist;
import fidas.server.gameserver.datatables.EventDroplist.DateDrop;
...
import fidas.server.gameserver.model.L2CommandChannel;
import fidas.server.gameserver.model.L2DropCategory;
import fidas.server.gameserver.model.L2DropData;
import fidas.server.gameserver.model.L2Object;
import fidas.server.gameserver.model.L2Party;
import fidas.server.gameserver.model.actor.instance.L2GrandBossInstance;
import fidas.server.gameserver.model.actor.instance.L2MonsterInstance;
...
import fidas.server.gameserver.model.actor.knownlist.AttackableKnownList;
import fidas.server.gameserver.model.actor.status.AttackableStatus;
...
import fidas.server.gameserver.taskmanager.DecayTaskManager;
...
public class L2Attackable extends L2Npc
{
	...
	protected void calculateRewards(L2Character lastAttacker)
	{
		...
		try
		{
					...
					if (attackerParty == null)
					{
						...
						if (attacker.getKnownList().knowsObject(this))
						{
							...
							if (attacker instanceof L2PcInstance)
							{
								...
								//FIXME VIP
								if (((L2PcInstance) attacker).isVip() && Config.ALLOW_VIP_EXP_SP)
								{
								    exp *= Config.VIP_XP;
								    sp *= Config.VIP_SP;
								}
			...
			// FIXME VIP+ Desaparece el bicho cuando es VIP+  (aio)
			//if (lastAttacker.getActingPlayer().isAio())
			//{
			//	this.endDecayTask();
			//}
	 ...
	 * @return
	 */
	private RewardItem calculateRewardItem(L2PcInstance lastAttacker, L2DropData drop, int levelModifier, boolean isSweep, boolean autoSweepAio, boolean isChampionPersonalReward)
	{
	    double dropChance = drop.getChance();
	    int deepBlueDrop = 1;

	    // Deep blue: NO aplicarlo a personal reward champion
	    if (!isChampionPersonalReward)
	    {
	    ...
	    // Champion normal: NO reaplicar si ya es personal reward champion
	    if (!isChampionPersonalReward && isChampion())
	    {
	    ...
	    if (isSweep)
	    {
	        if (Config.ALLOW_VIP_SPOIL)
	        {
	            // Sweep manual
	            if (!autoSweepAio && lastAttacker.isVip())
	            {
	                dropChance *= Config.VIP_SPOIL;
	            }
	            // Autospoil AIO: se ajusta abajo sobre itemCount
	        }
	    }
	    else
	    {
	        if (lastAttacker.isVip() && Config.ALLOW_VIP_DROP)
	        {
	            dropChance *= Config.VIP_DROP;
	        }
	    }
	    ...
	    // Champion adena extra: NO reaplicar si ya es personal reward champion
	    if (!isChampionPersonalReward && isChampion() && ((drop.getItemId() == PcInventory.ADENA_ID) || Util.contains(SevenSigns.SEAL_STONE_IDS, drop.getItemId())))
	    {
	        itemCount *= ChampionData.getInstance().getMultiplier(getChampionType(), "bonusDropAdena");
	    }

	    // VIP adena
	    if ((lastAttacker.isVip() && Config.ALLOW_VIP_ADENA) && ((drop.getItemId() == PcInventory.ADENA_ID) || Util.contains(SevenSigns.SEAL_STONE_IDS, drop.getItemId())))
	    {
	        itemCount = Math.round(itemCount * Config.VIP_ADENA);
	    }

	    // VIP+ autospoil híbrido SOLO para autospoil AIO
	    if (isSweep && autoSweepAio && Config.ALLOW_VIP_SPOIL)
	    {
	        if (itemCount > 1)
	        {
	            itemCount = Math.round(itemCount * Config.VIPPLUS_SPOIL);
	            if (itemCount < 1)
	            {
	                itemCount = 1;
	            }
	        ...
	        {
	            if (Rnd.get(1000) >= (int) (Config.VIPPLUS_SPOIL * 1000))
	            {
	                itemCount = 0;
	            }
	        }
	 ...
	 * @return
	 */
	private RewardItem calculateRewardItem(L2PcInstance lastAttacker, L2DropData drop, int levelModifier, boolean isSweep, boolean autoSweepAio)
	{
		return calculateRewardItem(lastAttacker, drop, levelModifier, isSweep, autoSweepAio, false);
	}
	...
	private RewardItem calculateCategorizedRewardItem(L2PcInstance lastAttacker, L2DropCategory categoryDrops, int levelModifier)
	{
		...
		if (lastAttacker.isVip() && Config.ALLOW_VIP_DROP)
		{
			categoryDropChance *= Config.VIP_DROP;
		}
	...
	public void doItemDrop(L2NpcTemplate npcTemplate, L2Character mainDamageDealer)
	{
		...
		for (L2DropCategory cat : npcTemplate.getDropData())
		{
			...
			if (cat.isSweep())
			{
			    if (isSpoil() || player.isAio())
			    {
			        final boolean autoSweepAio = player.isAio() && !isSpoil();
			        FastList<RewardItem> sweepList = autoSweepAio ? null : new FastList<>();

			        _log.info("[VIP-SWEEP-CONTEXT] mob=" + getName()
			            + " player=" + player.getName()
			            + " isSpoilMob=" + isSpoil()
			            + " spoiledBy=" + getIsSpoiledBy()
			            + " isAio=" + player.isAio()
			            + " autoSweepAio=" + autoSweepAio);

			        for (L2DropData drop : cat.getAllDrops())
			        {
			            item = calculateRewardItem(player, drop, levelModifier, true, autoSweepAio, false);
			            if (item == null)
			            {
			                continue;
			            }
			            ...
			            if (autoSweepAio)
			            {
			                FastList<L2Item> curItems = new FastList<>();
			                curItems.add(ItemTable.getInstance().createDummyItem(item.getItemId()).getItem());
			                boolean canSweep = player.getInventory().checkInventorySlotsAndWeight(curItems, true, false);

			                _log.info("[VIP-SWEEP-AIO-GIVE] player=" + player.getName()
			            ...
			            {
			                _log.info("[VIP-SWEEP-NORMAL-LIST] player=" + player.getName()
			                    + " itemId=" + item.getItemId()
			                    + " count=" + item.getCount());
			        ...
			        if (!autoSweepAio && (sweepList != null) && !sweepList.isEmpty())
			        {
			...
			else
			{
			    if (isSeeded())
			    {
			        ...
			        item = calculateRewardItem(player, drop, levelModifier, false, false, false);
			    }
			    else
			    {
		...
		if (isChampion() && ChampionData.getInstance().hasPersonalReward(getChampionType()) && (((getLevel() + Config.L2JMOD_CHAMPION_LVL_DIFF) >= player.getLevel()) || (Config.L2JMOD_CHAMPION_REWARD_LOWER_LVL_ITEM_CHANCE > 0)))
		{
		    ...
		    {
		        RewardItem item = calculateRewardItem(player, reward, 0, false, false, true);

...
import fidas.server.gameserver.handler.ChatHandler;
...
public final class Say2 extends L2GameClientPacket
{
	...
	// VIP chat reuses battlefield type
	public static final int VIP_CHAT = BATTLEFIELD; // &
	...
	protected void runImpl()
	{
		...
		if (!isValidVoicedCommand(_text) && ((_type == ALL) || (_type == SHOUT)) && !activeChar.isGM() && (activeChar.getLevel() < Config.GLOBAL_CHAT_MIN_LEVEL)) // BLOQUEA POR NIVEL TANTO CHAT GENERAL COMO SHOUT (!) PERO PERMITE COMANDOS (.test)
		{
			activeChar.sendMessage("You must be level " + Config.GLOBAL_CHAT_MIN_LEVEL + " or higher to use this chat.");
			return;
		}

		if ((_type == TRADE) && !activeChar.isGM() && (activeChar.getLevel() < Config.TRADE_CHAT_MIN_LEVEL)) // BLOQUEA POR NIVEL EL CHAT TRADE (+)
		{
			activeChar.sendMessage("You must be level " + Config.TRADE_CHAT_MIN_LEVEL + " or higher to use trade chat.");
			return;
		}
		...
		if (!activeChar.isGM())
		{
			if ((_type == SHOUT) || (_type == TRADE))
			{
				if (!getClient().getFloodProtectors().getGlobalChat().tryPerformAction("global chat"))
				{
					activeChar.sendMessage(getFloodWaitMessage(Config.FLOOD_PROTECTOR_GLOBAL_CHAT.FLOOD_PROTECTION_INTERVAL));
					return;
				}
			}
			else if (_type == HERO_VOICE)
			{
				...
				{
					activeChar.sendMessage(getFloodWaitMessage(Config.FLOOD_PROTECTOR_HERO_VOICE.FLOOD_PROTECTION_INTERVAL));
					return;
				}
			}
			else if (_type == VIP_CHAT)
			{
				if (!activeChar.isVip())
				{
					activeChar.sendMessage("Only VIP players can use the VIP chat.");
					return;
				}

				if (!getClient().getFloodProtectors().getVipChat().tryPerformAction("vip chat"))
				{
					activeChar.sendMessage(getFloodWaitMessage(Config.FLOOD_PROTECTOR_VIP_CHAT.FLOOD_PROTECTION_INTERVAL));
					return;
				}
			}
		}
	...
	private String getFloodWaitMessage(int floodInterval)
	{
		// Flood protector intervals are configured in 100 ms ticks.
		int seconds = (int) Math.ceil(floodInterval / 10.0);
		...
		return "You are talking too fast. Please wait " + seconds + " second" + (seconds > 1 ? "s" : "") + ".";
	}
	...
	private boolean isValidVoicedCommand(String text)
	{
		...
		// Must not contain spaces (single word)

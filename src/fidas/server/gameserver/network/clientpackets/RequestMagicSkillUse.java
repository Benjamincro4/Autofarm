...
public final class RequestMagicSkillUse extends L2GameClientPacket
{
	...
	protected void runImpl() {
	    ...
	    if (skill != null) {
	        // Verifica si hay un slot pendiente
	        if (activeChar.getPendingAutoFarmSlot() != null) {
	            String slot = activeChar.getPendingAutoFarmSlot();

	            // Asigna la habilidad usando AutoFarmManager
	            AutoFarmManager.assignSkillToSlot(activeChar, slot, skill.getId());
	            activeChar.setPendingAutoFarmSlot(null);

	            AutoFarmManager.refreshAutoFarmHtml(activeChar); // Actualiza el HTML
	            return;
	        }

	        // Ejecuta la habilidad normalmente si no hay slot pendiente

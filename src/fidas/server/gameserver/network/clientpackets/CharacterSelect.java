...
import fidas.server.gameserver.model.CharSelectInfoPackage;
...
import fidas.server.gameserver.network.serverpackets.CharSelected;
import fidas.server.gameserver.network.serverpackets.NpcHtmlMessage;
import fidas.server.gameserver.network.serverpackets.SSQInfo;
		...
		if (client.getActiveCharLock().tryLock())
		{
			try
			{
				...
				if (client.getActiveChar() == null)
				{
					...
					boolean isVip = false;
					int charId = client.getCharSelection(_charSlot).getObjectId();
					L2PcInstance player = L2PcInstance.restore(charId);
					if (player != null)
					{
						isVip = player.isVip() || (client.getCharSelection(_charSlot).getAccessLevel() == 8);
						player.deleteMe();
					}

					final boolean dualboxChecked = (Config.L2JMOD_DUALBOX_CHECK_MAX_PLAYERS_PER_IP > 0);

					if (dualboxChecked && !AntiFeedManager.getInstance().tryAddClient(
							AntiFeedManager.GAME_ID,
							client,
							Config.L2JMOD_DUALBOX_CHECK_MAX_PLAYERS_PER_IP,
							isVip))
					{
						final NpcHtmlMessage msg = new NpcHtmlMessage(0);
						msg.setFile(info.getHtmlPrefix(), "data/html/mods/IPRestrictionVIP.htm");

						final int limit = AntiFeedManager.getInstance().getLimit(client, Config.L2JMOD_DUALBOX_CHECK_MAX_PLAYERS_PER_IP);
						msg.replace("%max%", (limit < 0) ? "unlimited" : String.valueOf(limit));
